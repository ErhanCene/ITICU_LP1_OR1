...
for (m in methods){
  ...
  for (opt in optimizers[optimizers != "none"]){
    ...
    # NEW: for MODI, also export the final objective as RAW LaTeX (in its own HTML)
    if (opt == "modi") {
      transport_export_latex_equation_html(
        cost  = res_opt$cost,
        alloc = res_opt$allocation,
        file  = file.path(outdir, sprintf("21_%s_modi_final_cost_latex.html", m)),
        title = sprintf("%s – MODI Nihai Toplam Maliyet (LaTeX – ham)", method_title(m)),
        render_math = FALSE   # set TRUE if you also want MathJax preview
      )
    }  
  }
}
...
build_transport_summary_index <- function(C, sup, dem, outdir,
  ...
  for (m in methods) {
    ...
    # Link to the MODI final cost LaTeX page, if it exists
    modi_final_eq <- sprintf("21_%s_modi_final_cost_latex.html", m)
    if (file.exists(file.path(outdir, modi_final_eq))) {
      lines <- c(lines,
                 sprintf("<li><a href='%s'>%s</a></li>",
                         modi_final_eq, H("MODI Nihai Maliyet Eşitliği (LaTeX – raw)")))
    }
...
lbl <- list(
  ...
  col_names = c("Depo 1","Depo 2","Depo 3","Dummy"),
  supply_label = "Arz",
  demand_label = "Talep",
  initial_title = "Başlangıç Tablosu (Birim Maliyetler)",
  allocation_title = "Dağıtım – %METHOD%",
...
# ============================================================
lbl <- list(
  row_names = c("Fabrika 1","Fabrika 2","Fabrika 3", 'Dummy'),
