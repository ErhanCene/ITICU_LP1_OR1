...
render_modi_uv_on_final <- function(tr, final_res, outdir,
  ...
  render_uv_alloc_table <- function(A, u, v, caption = NULL, file, basis = NULL) {
    if (is.null(basis)) basis <- (A > 0)   # fallback
    deg_mask <- (basis == 1) & (A == 0)
    
    html <- c("<html><head><meta charset='utf-8'>", base_css,
              "<style>
               /* degenerate basic (joker) cell style */
               .deg{background:#f3e8ff;border:2px dashed #7c3aed}
               .deg .badge{display:inline-block;border:1px solid #7c3aed;
                           border-radius:999px;font-size:11px;padding:0 6px;
                           margin-right:4px;color:#7c3aed;background:#fff}
             </style>",
              "</head><body>")
    html <- c(html, sprintf("<table class='tbl-sim'><caption>%s</caption>",
                            H(ifelse(is.null(caption), "", caption))))
    # header with v ...
    html <- c(html, "<thead><tr><th></th>")
    for (j in seq_len(n)) {
    ...
    for (i in seq_len(m)) {
      ...
      for (j in seq_len(n)) {
        ...
        if (deg_mask[i,j]) cls <- c(cls, "deg")   # <-- dejenere vurgusu
        
        # içerik
        if (A[i,j] > 0) {
          cell <- sprintf("<div class='big'>%g</div><div class='sub'>(c=%g)</div>", A[i,j], C[i,j])
        } else if (deg_mask[i,j]) {
          # joker 0-tahsis temel → rozet + cost
          cell <- sprintf("<span class='badge'>deg</span><div class='muted'>&ndash;</div><div class='sub'>(c=%g)</div>", C[i,j])
        } else {
          cell <- sprintf("<div class='muted'>&ndash;</div><div class='sub'>(c=%g)</div>", C[i,j])
        }
        html <- c(html, sprintf("<td class='%s'>%s</td>", paste(cls, collapse=" "), cell))
      }
    ...
    # küçük açıklama
    html <- c(html, "<p class='sub'>Mor kesik çerçeve <b>dejenere temel</b> (basis=1, x=0) hücreyi gösterir.</p>")
  ...
  while (i <= length(steps)) {
    ...
    render_uv_alloc_table(A_before, u, v, caption = cap, file = f_uv, basis = is_basic_before)
...
# ============================================================
# Export RAW LaTeX for u_i and v_j values (per MODI step)
#  - u, v: numeric vectors (may include NA)
#  - file: output HTML path (raw LaTeX is embedded; optional MathJax preview)
#  - also writes a sibling .tex file with only the LaTeX
# ============================================================
transport_export_latex_uv_values_from_step <- function(u, v, file,
                                                       title = "LaTeX – u_i ve v_j Değerleri",
                                                       render_math = FALSE,
                                                       mathjax_src = "https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-mml-chtml.js",
                                                       include_na = FALSE){
  .mf <- function(x){
  ...
  for (i in seq_along(u)){
    if (!is.na(u[i]) || include_na){
      rhs <- if (is.na(u[i])) "\\text{NA}" else .mf(u[i])
      u_lines <- c(u_lines, sprintf("u_{%d} &= %s \\\\", i, rhs))
    }
  }
  if (length(u_lines)) u_lines[length(u_lines)] <- sub("\\\\\\\\$", "", u_lines[length(u_lines)])
  
  # Lines for v_j = ...
  v_lines <- character(0)
  for (j in seq_along(v)){
    if (!is.na(v[j]) || include_na){
      rhs <- if (is.na(v[j])) "\\text{NA}" else .mf(v[j])
      v_lines <- c(v_lines, sprintf("v_{%d} &= %s \\\\", j, rhs))
    }
  }
  if (length(v_lines)) v_lines[length(v_lines)] <- sub("\\\\\\\\$", "", v_lines[length(v_lines)])
  
  # Vector forms (only defined entries)
  u_def <- if (all(is.na(u))) NULL else paste0(sapply(u[!is.na(u)], .mf), collapse="\\;")
  v_def <- if (all(is.na(v))) NULL else paste0(sapply(v[!is.na(v)], .mf), collapse="\\;")
  vec_block <- NULL
  if (!is.null(u_def) || !is.null(v_def)){
    vec_u <- if (is.null(u_def)) "" else sprintf("\\mathbf{u} &= \\begin{bmatrix}%s\\end{bmatrix}", u_def)
    vec_v <- if (is.null(v_def)) "" else sprintf("\\mathbf{v} &= \\begin{bmatrix}%s\\end{bmatrix}", v_def)
    vec_block <- paste0("\\[\\begin{aligned}\n",
                        paste(c(vec_u, vec_v)[nzchar(c(vec_u, vec_v))], collapse=" \\\\\n"),
                        "\n\\end{aligned}\\]\n")
  }
  
  # Combined LaTeX (scalar equalities)
  scalars_block <- paste0(
    "\\[\\begin{aligned}\n",
    if (length(u_lines)) paste(u_lines, collapse="\n") else "\\text{(u değerleri tanımsız)}",
    "\n\\end{aligned}\\]\n",
    "\n",
    "\\[\\begin{aligned}\n",
    if (length(v_lines)) paste(v_lines, collapse="\n") else "\\text{(v değerleri tanımsız)}",
    "\n\\end{aligned}\\]\n"
  )
  
  raw_latex <- paste0(scalars_block, if (!is.null(vec_block)) vec_block else "")
  
  # also write a pure .tex next to the HTML
  tex_path <- sub("\\.html?$", ".tex", file, ignore.case = TRUE)
  dir.create(dirname(file), showWarnings = FALSE, recursive = TRUE)
  writeLines(raw_latex, tex_path)
  
  # minimal HTML wrapper showing RAW LaTeX (+ optional MathJax preview)
  ...
  html <- c(html, "</head><body>",
            sprintf("<h3>%s</h3>", H(title)),
            sprintf("<div class='raw'>%s</div>", H(raw_latex)))
  if (render_math){
    html <- c(html, "<h4>Önizleme (MathJax)</h4>", raw_latex)
  }
  html <- c(html,
            sprintf("<div class='note'>Ayrı .tex dosyası: <code>%s</code></div>", H(basename(tex_path))),
            "</body></html>")
  writeLines(html, file)
  invisible(raw_latex)
}
...
render_modi_step_packages <- function(tr, outdir, method_label,
  ...
  base_css <- paste0(
    ...
    /* NEW: degenerate basic (basis=1 & x=0) */
    .deg{
      background:#f3e8ff !important;                 /* soft purple */
      border:2px dashed #7c3aed !important;
    }
    .deg .badge{
      display:inline-block;border:1px solid #7c3aed;border-radius:999px;
      font-size:11px;padding:0 6px;margin-right:4px;color:#7c3aed;background:#fff
    }
  </style>"
  )
  ...
  write_uv_table <- function(step_id, st){
    ...
    u <- st$info$u; v <- st$info$v
    basis <- if (!is.null(st$info$basis)) (st$info$basis == 1) else (A > 0)
    deg_mask <- (basis & (A == 0))
    m <- nrow(C); n <- ncol(C)
    lbl <- modifyList(.transport_labels_default(), labels_for_steps, keep.null = TRUE)
    ...
    for (i in 1:m){
      ...
      for (j in 1:n){
        cls <- character(0)
        if (A[i,j] > 0) cls <- c(cls, "alloc")
        if (deg_mask[i,j]) cls <- c(cls, "deg")
        
        if (A[i,j] > 0){
          cell <- sprintf("<div class='big'>%g</div><div class='sub'>(m=%g)</div>", A[i,j], C[i,j])
        } else if (deg_mask[i,j]) {
          cell <- sprintf("<span class='badge'>deg</span><div class='muted'>&ndash;</div><div class='sub'>(m=%g)</div>", C[i,j])
        } else {
          cell <- sprintf("<div class='muted'>&ndash;</div><div class='sub'>(m=%g)</div>", C[i,j])
        }
        html <- c(html, sprintf("<td class='%s'>%s</td>", paste(cls, collapse=" "), cell))
  ...
  write_uvR_table <- function(step_id, st){
    ...
    for (i in 1:m){
      ...
      for (j in 1:n){
        ...
        if (deg_mask[i,j]) cls <- c(cls, "deg")   # NEW
    ...
    # NEW: u_i and v_j derivations in OCCURRENCE ORDER (from uv_log)
    uv_eqs_file <- sprintf("50_%s_step_%03d_uv_eqs.html", method_label, step_id)
    transport_export_latex_uv_eqs_from_step(
      u = u, v = v, C = C, uv_log = st$info$uv_log,
      file = file.path(outdir, uv_eqs_file),
      title = "u–v türetimleri (LaTeX – ham, sırayla)",
      render_math = FALSE
    )
  ...
  for (st in tr$steps){
    ...
    f3 <- sprintf("50_%s_step_%03d_uv_eqs.html", method_label, snap_id)
    idx <- c(idx,
             sprintf("<li>Adım %d: <a href='%s'>u/v tablo</a> · <a href='%s'>u/v + R<sub>ij</sub></a> · <a href='%s'>u,v (LaTeX)</a></li>",
                     st$k, f1, f2,
                     sprintf("50_%s_step_%03d_uv_values.html", method_label, snap_id)))
...
# ============================================================
# Alternative optimal solutions when MODI stops with R_ij = 0 (non-basic)
# Produces per-candidate HTML:
#  - cycle transition (before/after)
#  - alternative allocation table
#  - raw LaTeX for the alternative objective
# And an index page listing all alternatives found.
# ============================================================
render_modi_alternative_solutions <- function(tr, outdir, method_label,
  ...
  # ---------- find the final MODI snapshot where we concluded optimality ----------
  # We want the *last* "modi-uv" step. That snapshot has u, v, R and the current allocation.
  last_uv <- NULL
  for (st in rev(tr$steps)) {
    if (identical(st$stage, "modi-uv")) { last_uv <- st; break }
  }
  if (is.null(last_uv)) {
    warning("No MODI snapshot found; nothing to do.")
  ...
  u <- last_uv$info$u
  v <- last_uv$info$v
  R <- last_uv$info$reduced
  
  # If R is missing, bail.
  if (is.null(R) || all(is.na(R))) {
    warning("R matrix not available in the last MODI snapshot.")
    return(invisible(FALSE))
  }
  
  # candidates = non-basic cells with A[i,j]==0 and |R| <= tol_zero
  # Prefer basis mask if present in snapshot; otherwise treat basic as (A>0 or degenerate basic if provided elsewhere)
  basis <- last_uv$info$basis
  if (is.null(basis)) basis <- (A > 0) + 0
  
  cand <- which((basis == 0) & (A == 0) & (!is.na(R)) & (abs(R) <= tol_zero), arr.ind = TRUE)
  if (!nrow(cand)) {
    # Nothing to show -> write an index that says none
    idx_file <- file.path(outdir, sprintf("47_%s_alternatives_index.html", method_label))
    writeLines(c(
      "<html><head><meta charset='utf-8'>",
      .transport_css_bigM(),
      "</head><body>",
      sprintf("<h2>%s – Alternatif Optimal Çözümler</h2>", H(method_label)),
      "<p>Bu örnekte R_{ij}=0 olan uygun olmayan (non-basic) hücre bulunamadı; alternatif optimal çözüm yok.</p>",
      "</body></html>"
    ), idx_file)
    return(invisible(TRUE))
  }
  
  # ---------- helpers reused from your overlay renderer ----------
  eps <- if (exists(".eps", inherits = FALSE)) get(".eps") else 1e-9
  
  find_loop_local <- function(is_basic, ie, je) {
    # same logic as your find_loop() in render_modi_uv_on_final
    rows_to_cols <- lapply(seq_len(m), function(i) which(is_basic[i, ]))
    cols_to_rows <- lapply(seq_len(n), function(j) which(is_basic[, j]))
    start <- ie; target <- m + je; N <- m + n
    prev <- rep(NA_integer_, N); visited <- rep(FALSE, N)
  ...
  apply_cycle_once <- function(A0, loop_path) {
    signs <- rep(c(1, -1), length.out = nrow(loop_path))
    minus <- loop_path[signs == -1, , drop = FALSE]
    if (!nrow(minus)) return(list(A=A0, theta=0, left=NULL))
    theta <- suppressWarnings(min(A0[cbind(minus[,1], minus[,2])], na.rm = TRUE))
    if (!is.finite(theta)) theta <- 0
    A1 <- A0
    if (theta > eps) {
      for (k in seq_len(nrow(loop_path))) {
        i0 <- loop_path[k,1]; j0 <- loop_path[k,2]
        A1[i0, j0] <- A1[i0, j0] + signs[k] * theta
        if (abs(A1[i0, j0]) < 1e-12) A1[i0, j0] <- 0
      }
    }
    list(A=A1, theta=theta)
  }
  
  # simple rendering for transition before/after (reuses your visual language)
  render_transition_simple <- function(A_before, A_after, ie, je, loop_path, file, title) {
    base_css <- paste0(
      .transport_css_bigM(),
      "<style>
        ...
        .pm{display:inline-block;border:1px solid #ced4da;border-radius:999px;padding:0 5px;font-size:12px;line-height:18px;min-width:18px;text-align:center;margin-right:4px;background:#fff}
        .pm.plus{border-color:#2e7d32;color:#2e7d32}
        .pm.minus{border-color:#c62828;color:#c62828}
        .cellwrap{position:relative;min-width:76px;height:46px}
        .corner-note{position:absolute;right:6px;top:6px}
    ...
    html <- c("<html><head><meta charset='utf-8'>", base_css, "</head><body>")
    html <- c(html, sprintf("<h3>%s</h3>", H(title)))
    html <- c(html, "<table class='tbl-sim'><thead><tr><th></th>")
    for (j in seq_len(n)) html <- c(html, sprintf("<th>%s</th>", H(col_names[j])))
    html <- c(html, "<th>Δ</th></tr></thead><tbody>")
    ...
    for (i in seq_len(m)) {
      ...
      for (j in seq_len(n)) {
        ...
        in_cycle <- is_pivot_cell(i, j) || is_plus(i, j) || is_minus(i, j)
        val_after <- A_after[i, j]; val_before <- A_before[i, j]
        if (in_cycle) row_delta <- row_delta + (val_after - val_before)
        cell_txt <- if (in_cycle || val_after > 0)
          sprintf("<div class='big'>%g</div>", val_after)
        else "<div class='muted'>&ndash;</div>"
        cell <- sprintf("<div class='cellwrap'><div class='corner-note'>c=%g</div>%s%s</div>",
                        C[i, j], badge, cell_txt)
        html <- c(html, sprintf("<td class='%s'>%s</td>", paste(cls, collapse=" "), cell))
      }
      html <- c(html, sprintf("<td class='rhs'>%s%g</td>", if (row_delta>0) "+" else "", row_delta), "</tr>")
    }
    html <- c(html, "</tbody></table></body></html>")
    writeLines(html, file)
  }
  
  write_alloc_table <- function(A_now, caption, file) {
    html <- transport_result_from_alloc_html(
      cost = C, alloc = A_now, title = caption, labels = lbl, include_css = TRUE
    )
    writeLines(html, file)
  }
  
  # ---------- loop over candidates and produce artifacts ----------
  items <- character(0)
  n_alts <- 0L
  for (k in seq_len(nrow(cand))) {
    if (n_alts >= max_alternatives) break
    ie <- cand[k, 1]; je <- cand[k, 2]
    
    loop <- find_loop_local(is_basic = (basis == 1), ie = ie, je = je)
    if (is.null(loop) || nrow(loop) < 4L) next   # cannot pivot along a cycle
    
    # apply cycle with theta = min(allocation on minus corners)
    res <- apply_cycle_once(A, loop)
    if (res$theta <= eps) next                   # degeneracy prevents moving -> skip
    
    A_alt <- res$A
    # same total cost (within tolerance)
    cost0 <- sum(A * C); cost1 <- sum(A_alt * C)
    if (!isTRUE(all.equal(cost0, cost1, tolerance = 1e-6))) {
      # In theory it must be equal if R_{ij}=0; keep it but add a small note
      warning(sprintf("Alternative #%d has slightly different total (%.10g vs %.10g).", n_alts+1, cost0, cost1))
    }
    
    n_alts <- n_alts + 1L
    tag <- sprintf("alt_%02d_i%d_j%d", n_alts, ie, je)
    
    # (1) Transition BEFORE -> AFTER
    f_before <- file.path(outdir, sprintf("47_%s_%s_transition_before.html", method_label, tag))
    f_after  <- file.path(outdir, sprintf("47_%s_%s_transition_after.html",  method_label, tag))
    render_transition_simple(A_before = A, A_after = A,  ie=ie, je=je, loop_path=loop,
                             file = f_before,
                             title = "")
    render_transition_simple(A_before = A, A_after = A_alt, ie=ie, je=je, loop_path=loop,
                             file = f_after,
                             title = '')
    
    # (2) Alternative allocation table
    f_alloc <- file.path(outdir, sprintf("47_%s_%s_allocation.html", method_label, tag))
    write_alloc_table(A_alt,
                      caption = sprintf("Alternatif Dağıtım — Pivot (%d,%d), Toplam = %g", ie, je, sum(A_alt * C)),
                      file = f_alloc
    )
    
    # (3) RAW LaTeX objective for the alternative
    f_eq <- file.path(outdir, sprintf("47_%s_%s_final_eq.html", method_label, tag))
    transport_export_latex_equation_html(
      cost = C, alloc = A_alt, file = f_eq,
      title = "Alternatif Optimal Çözüm — Toplam (LaTeX – ham)",
      render_math = render_math
    )
    
    items <- c(items, sprintf(
      "<li>Pivot aday: (%d,%d) — R=0 &middot; <a href='%s'>önce</a> / <a href='%s'>sonra</a> · <a href='%s'>dağıtım</a> · <a href='%s'>LaTeX</a></li>",
      ie, je,
      basename(f_before), basename(f_after),
      basename(f_alloc), basename(f_eq)
    ))
  }
  ...
  if (!length(items)) {
    writeLines(c(
      "<html><head><meta charset='utf-8'>", .transport_css_bigM(), "</head><body>",
      sprintf("<h2>%s – Alternatif Optimal Çözümler</h2>", H(method_label)),
      "<p>R_{ij}=0 koşulunu sağlayan uygun olmayan hücrelerle geçerli bir çevrim kurulamıyor; alternatif çözüm üretilmedi.</p>",
      "</body></html>"
    ), idx_file)
  } else {
    writeLines(c(
      "<html><head><meta charset='utf-8'>", .transport_css_bigM(), "</head><body>",
      sprintf("<h2>%s – Alternatif Optimal Çözümler</h2>", H(method_label)),
      "<p>MODI optimumunda R_{ij}=0 olan aşağıdaki hücreler üzerinden alternatif çözümler:</p>",
      "<ol>", items, "</ol>",
      "</body></html>"
    ), idx_file)
  }
