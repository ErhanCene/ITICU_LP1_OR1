...
# ---------- Fallback helpers (only created if absent) ----------
...
# ---------- Fallback: step view with (mevcut / toplam) ----------
if (!exists("transport_result_from_alloc_step_html", mode = "function")) {
  transport_result_from_alloc_step_html <- function(cost, alloc,
    ...
    for (i in seq_len(m)) {
      ...
      for (j in seq_len(n)) {
        ...
        if (A[i,j] > 0){
          cell <- sprintf("<div class='bold'>%g</div><div class='sub'>(m=%g)</div>", A[i,j], C[i,j])
...
# --- Minimal step-by-step renderer (before/after + index) ---
render_trace_steps <- function(tr, dataset_title, outdir, method_label,
  ...
  for (t in seq_along(tr$steps)) {
    ...
    # reconstruct a simple “before” by undoing the last shipment if recorded
    A_after  <- st$alloc + 0
    A_before <- A_after
    ...
    # titles
    step_title <- paste0("Adım ", st$k, if (!is.null(st$info$note)) paste0(" — ", st$stage, ": ", st$info$note) else paste0(" — ", st$stage))
...
render_modi_uv_on_final <- function(tr, final_res, outdir,
  ...
  render_R_table <- function(u,v,file){
    ...
    for (i in seq_len(m)){
      ...
      for (j in seq_len(n)){
        ...
        if (!is.na(val) && val < 0) cls <- c(cls,"neg")
        if (is.finite(minR) && !is.na(val) && abs(val-minR)<=1e-12) cls <- c(cls,"minneg")
  ...
  render_transition <- function(A_after, A_before, ie, je, loop_path, file){
    ...
    in_plus  <- function(i,j) any(plus[,1]==i & plus[,2]==j)
    in_minus <- function(i,j) any(minus[,1]==i & minus[,2]==j)
    html <- c("<html><head><meta charset='utf-8'>", css, "</head><body>",
              "<table class='tbl-sim'><thead><tr><th></th>")
    for (j in seq_len(n)) html <- c(html, sprintf("<th>%s</th>", H(cn[j])))
    html <- c(html, "</tr></thead><tbody>")
    for (i in seq_len(m)){
      ...
      for (j in seq_len(n)){
        cls <- character(0)
        if (is_pivot(i,j)) cls <- c(cls, "pivot") else
          if (in_plus(i,j))  cls <- c(cls, "plus")  else
            if (in_minus(i,j)) cls <- c(cls, "minus")
        val <- A_after[i,j]
        before <- A_before[i,j]
        ...
        html <- c(html, sprintf("<td class='%s'><div class='cellwrap'><div class='cbox'>c=%g</div>%s</div></td>",
  ...
  for (p in seq_len(nrow(zero_nb))){
    ...
    for (i in seq_len(m)){
      ...
      for (j in seq_len(n)){
        ...
        cls <- if (A_alt[i,j] > 0) "alloc" else ""
        if (in_loop) cls <- paste(cls, if (any(minus[,1]==i & minus[,2]==j)) "minus" else "plus")
        if (A_alt[i,j] > 0) {
...
render_modi_step_packages <- function(tr, outdir, method_label,
  ...
  for (st in tr$steps){
    ...
    is_pivot <- matrix(FALSE, m, n)
    if (is.finite(minR) && minR < 0){
      w <- which(!is.na(R)&abs(R-minR)<=1e-12, arr.ind=TRUE)
      if (nrow(w)) is_pivot[cbind(w[,1],w[,2])] <- TRUE
    }
    ...
    for (i in 1:m){
      ...
      for (j in 1:n){
        if (A[i,j] > 0) {
          cell <- sprintf("<div class='bold'>%g</div><div class='sub'>(c=%g)</div>", A[i,j], C[i,j])
          # degenerate basic marker
          cls <- if (A[i,j]==0) "alloc degen" else "alloc"
          html <- c(html, sprintf("<td class='%s'>%s</td>", cls, cell))
        } else {
          html <- c(html, sprintf("<td><div class='muted'>&ndash;</div><div class='sub'>(c=%g)</div></td>", C[i,j]))
        }
    ...
    for (i in 1:m){
      ...
      for (j in 1:n){
        ...
        if (!is.na(r) && r < 0) cls <- c(cls, "neg")
        if (is_pivot[i,j]) cls <- c(cls, "pivot")
        atop <- if (A[i,j] > 0) sprintf("<div class='bold'>%g</div>", A[i,j]) else "<div class='muted'>&ndash;</div>"
